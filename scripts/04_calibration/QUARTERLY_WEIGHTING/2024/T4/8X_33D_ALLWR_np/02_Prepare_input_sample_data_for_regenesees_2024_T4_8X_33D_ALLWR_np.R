...
tmpSD$X2[ tmpSD$AgeAnnee>= 15 & tmpSD$AgeAnnee <=34 & tmpSD$M5==1] <- 1
tmpSD$X3[ tmpSD$AgeAnnee>= 35 & tmpSD$AgeAnnee <=64 & tmpSD$M5==1] <- 1
tmpSD$X4[ tmpSD$AgeAnnee>= 65                       & tmpSD$M5==1] <- 1


tmpSD$X5[tmpSD$AgeAnnee>=  0 & tmpSD$AgeAnnee <=14 & tmpSD$M5==2] <- 1
tmpSD$X6[tmpSD$AgeAnnee>= 15 & tmpSD$AgeAnnee <=34 & tmpSD$M5==2] <- 1
tmpSD$X7[tmpSD$AgeAnnee>= 35 & tmpSD$AgeAnnee <=64 & tmpSD$M5==2] <- 1
tmpSD$X8[tmpSD$AgeAnnee>= 65                       & tmpSD$M5==2] <- 1
...
list_of_X <- c("X1","X2","X3","X4","X5","X6","X7","X8")  
list_of_X 
...
save(LFS_SAMPLE_DATA, file="LFS_SAMPLE_DATA_2024_T4_8X_33D_ALLWR_np.RData")


### If we have not set the working directory for the outputs (or we need to save the object in a different folder) we can use 

# save(LFS_SAMPLE_DATA ,file='D:/DOCUMENTS/CAE/Calibration/Applications ENEM/DATA/QUARTERLY_WEIGHTING/2024/T4/8X_33D_ALLWR_np//LFS_SAMPLE_DATA_2024_T4_8X_33D_ALLWR_np.RData')
...
save(LFS_SAMPLE_DATA_SUMMARY_OF_Xs_SAMPLE_SIZE, file="LFS_SAMPLE_DATA_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_Xs_SAMPLE_SIZE.RData")


###   Save the results in an excel file using the package "writexl"

write_xlsx(LFS_SAMPLE_DATA_SUMMARY_OF_Xs_SAMPLE_SIZE,
          "LFS_SAMPLE_DATA_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_Xs_SAMPLE_SIZE.xlsx" )
