...
load("LFS_SAMPLE_DATA_2024_T4_8X_33D_ALLWR_np.RData")
...
load("LFS_KNOWN_TOTALS_2024_T4_8X_33D_ALLWR_np.RData")
...
sample_data$PSUKEY    <- as.factor(paste0(sample_data$HH2, "-", sample_data$HH8))
sample_data$HHKEY     <- as.factor(paste0(sample_data$HH2, "-", sample_data$HH8, "-", sample_data$interview__key))
sample_data$INDKEY    <- as.factor(paste0(sample_data$HH2, "-", sample_data$HH8, "-", sample_data$interview__key, "-", sample_data$membres__id))
...
sum(rowSums( known_totals[, seq(2,9) ] ))
...
source("D:/DOCUMENTS/CAE/Calibration/Applications ENEM/PROG/QUARTERLY_WEIGHTING/Other_R_functions_for_Regenesees/Functions_to_Create _X_vector_and_X_Summary_Table.R")

###  use the function "constraints_model" to create the list of Xs identifying the calibration cells (Calibration model)

constrains_x <- constraints_model(8)
...
###  Check the total of the known total population 
sum(rowSums( popdataframe[, seq(2,9) ] ))
...
save(LFS_CALIBRATION_FINAL_WEIGHTS, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_FINAL_WEIGHTS.RData")
...
save(LFS_CALIBRATION_SUMMARY_OF_DESIGN_WEIGHTS, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_DESIGN_WEIGHTS.RData")

write.csv(LFS_CALIBRATION_SUMMARY_OF_DESIGN_WEIGHTS, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_DESIGN_WEIGHTS.csv")
...
save(LFS_CALIBRATION_SUMMARY_OF_FINAL_CORR_FACTOR, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_CORR_FACTORS.RData")

write.csv(LFS_CALIBRATION_SUMMARY_OF_FINAL_CORR_FACTOR, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_CORR_FACTORS.csv")
...
save(LFS_CALIBRATION_SUMMARY_OF_FINAL_WEIGHTS, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_FINAL_WEIGHTS.RData")

write.csv(LFS_CALIBRATION_SUMMARY_OF_FINAL_WEIGHTS, file="LFS_CALIBRATION_2024_T4_8X_33D_ALLWR_np_SUMMARY_OF_FINAL_WEIGHTS.csv")
