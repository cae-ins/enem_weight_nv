...
*Note : Dans le code, la formule est implémentée avec un multiplicateur $k$ qui s'annule mathématiquement mais explicite le tirage.*

*   **Si Région = ABIDJAN** ($k=104$) :
    $$ \pi_{ZD} = 104 \times \frac{N_{ménages\_ZD}}{N_{ménages\_strat}} \times \frac{N_{ZD\_strat}}{104} $$
*   **Si Autre Région** ($k=56$) :
    $$ \pi_{ZD} = 56 \times \frac{N_{ménages\_ZD}}{N_{ménages\_strat}} \times \frac{N_{ZD\_strat}}{56} $$
...
> $$ N_{ménages\_ZD} \approx N_{ménages\_segment} \times 6 $$
...
$$ \sum_{i \in \text{Echantillon}} w_{final, i} \times X_{i} = \text{Total}_{Population}(X) $$
...
*   **Logique** :
    ...
    2.  Agrège ce potentiel et le nombre réel d'enquêtés par `region` et `milieu`.
